...
- [사용자 관점 — 2가지 Pain Point 축](#사용자-관점--2가지-pain-point-축)
  - [① 클라우드 사용자 (개인 / 기업)](#-클라우드-사용자-관점--개인--기업)
  - [② 클라우드 서비스 제공자](#-클라우드-서비스-제공자-관점)
...
| 구분 | 내용 |
|------|------|
| **Who** (누가) | **① 클라우드 사용자** — 데이터를 계속 쌓기만 하는 개인 사용자와 저장 비용을 관리해야 하는 기업 담당자<br>**② 클라우드 서비스 제공자** — 고객 데이터를 보관하며 스토리지 효율을 책임지는 운영 담당자 |
...
## 사용자 관점 — 2가지 Pain Point 축

BIUM은 문제를 **두 개의 축**으로 나누어 정의했습니다.
데이터를 *쌓는 쪽*과 데이터를 *보관해주는 쪽*은 겪는 문제의 성격이 다르기 때문입니다.

| 축 | 대상 | 문제의 성격 | BIUM의 역할 |
|----|------|-------------|-------------|
| **① 클라우드 사용자** | 개인 사용자 · 기업 사용자 | 무엇을 지워야 할지 **판단하기 어렵다** | 정리 후보를 묶어 **근거와 함께 추천** |
| **② 클라우드 서비스 제공자** | 클라우드 운영 담당자 | 고객 데이터를 보관하면서 **비용을 최적화해야 한다** | **Cloud Storage Optimization Layer** |

---

## ① 클라우드 사용자 관점 — 개인 / 기업

### Persona A — Digital-Native Student (개인 사용자)
...
과제, 개발 프로젝트, 공모전, 대외활동을 하면서 중복 파일이 계속 쌓이지만,
제출 후 정리하는 과정이 귀찮아 **매월 불필요한 cloud 비용을 지불**합니다.

### Persona B — Game Data / Cloud Optimization Manager (기업 사용자)
...
### Pain Point → BIUM 솔루션 매칭

두 사용자는 **같은 네 가지 문제**를 서로 다른 규모로 겪습니다.

| 문제 | CS 전공 3학년<br>*Mr. 기찬아* | 게임회사 클라우드 담당자<br>*Mr. 김구름* | ✅ BIUM (우리 솔루션) |
|------|------|------|------|
| **무엇이 필요한지 알기 어렵다** | 사진·문서를 하나씩 열어봐야 삭제 여부를 알 수 있음 | 오래된 DB·모델·문서가 현재 업무에 필요한지 한눈에 파악하기 어려움 | **판단에 필요한 정보 제공**, 이에 따른 정리 서비스 |
| **데이터가 여러 곳에 흩어진다** | 스마트폰·PC·외장하드·iCloud·Google Drive 등에 같은 파일이 존재 | 개발 서버·클라우드·백업·프로젝트 저장소 등에 데이터가 분산 | **Local + Cloud를 함께 분석**해 동일·유사 데이터를 통합해서 보여줌 |
| **삭제하면 안 되는 데이터가 섞여 있다** | 추억 사진·중요 문서까지 실수로 삭제할까 걱정 | 업무 데이터·원본·백업 데이터를 잘못 삭제하면 복구 비용과 업무 리스크 발생 | 삭제 대상과 보호 대상을 구분하고, 근거를 제시한 뒤 **사용자가 결정** |
| **결국 계속 쌓인다** | 정리를 미루다가 클라우드 용량을 추가 구매 | 데이터 증가에 따라 스토리지 비용과 관리 부담이 지속 증가 | 중복·불필요 데이터를 선별해 **저장공간을 효율적으로 사용** |

---

## ② 클라우드 서비스 제공자 관점

### Persona C — Cloud Storage Optimization Manager
...
클라우드 운영 기업에서 BIUM은 **살짝 다른 역할**을 합니다.
사용자처럼 "무엇을 지울까"를 고민하는 것이 아니라,
**고객 데이터를 보관한 채로 저장 비용을 낮춰야** 합니다.

| 구분 | 내용 |
|------|------|
| **제약** | 고객 데이터는 임의로 삭제할 수 없음 — 안전한 보관이 최우선 |
| **목표** | 필요 이상의 비싼 스토리지 비용이 발생하지 않도록 최적화 |
| **한계** | 기존 정리 기준이 **파일의 실제 상태가 아닌 간접 지표**에 의존 |

### 기존 방식 — Google Lifecycle Policy

> 구글 `Lifecycle Policy` 공식 문서:
> *"객체를 가장 저렴한 저장 데이터 가격이 책정된 스토리지 클래스로 전환하는 `SetStorageClass` 작업이 우선합니다."*
>
> → 미사용 부분을 더 저렴한 스토리지 클래스로 이동하여 **비용 최적화**

**구글 클라우드 방식:** 사용 빈도·경과 시간 기반으로 정리 우선순위 추정

### ✅ BIUM의 해결 — Cloud Storage Optimization Layer

| 구분 | 정리 우선순위 판단 근거 | 한계 / 강점 |
|------|------------------------|-------------|
| **기존 Lifecycle Policy** | 사용 빈도 · 경과 시간 (**간접 지표**) | 오래 안 썼다는 이유만으로는 중복 여부·보존 가치를 알 수 없음 |
| **BIUM** | **실제 파일을 스윕해 상태를 파악** (해시·유사도·메타데이터) | 파일의 실제 중복·유사 상태를 근거로 삼아 판단 정확도를 높임 |

> **BIUM: 실제 파일을 스윕해 상태를 파악 → 사용자 판단 기반 우선순위 · storage 최적화**

시간 기반 추정이 아니라 **파일의 실제 중복·유사 상태**를 근거로 삼기 때문에,
저비용 스토리지 이동 판단의 정확도를 높이고, 삭제 없이도 비용을 낮출 수 있습니다.

---

## How Might We?

두 축의 Pain Point를 하나의 질문으로 수렴했습니다.
